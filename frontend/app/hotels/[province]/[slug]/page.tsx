...
export default async function HotelDetail({ params }: { params: { province: string; slug: string } }) {
  const res = await fetch(new URL(`/api/hotels/${params.province}/${params.slug}`, getBackendUrl()), { next: { revalidate: 0 } })
  const d = res.ok ? await res.json() : { name_th: decodeURIComponent(params.slug), lat: 18.82, lon: 98.96 }
  const lat = d.lat
  const lon = d.lon
  return (
    <div className="space-y-4">
      <div>
        <BackButton fallbackHref={`/hotels/${params.province}`} />
      </div>
