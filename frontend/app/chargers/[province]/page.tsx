...
export default async function ChargersProvince({ params, searchParams }: { params: { province: string }; searchParams?: { q?: string } }) {
  const q = searchParams?.q?.trim()
  const url = new URL('/api/chargers', getBackendUrl())
  url.searchParams.set('province', params.province)
  if (q) url.searchParams.set('q', q)
  const res = await fetch(url, { next: { revalidate: 0 } })
  const items: { id: string; name: string; lat: number; lon: number; type?: string; kw?: number }[] = res.ok ? await res.json() : []
  const markers = items.map((i) => ({ id: i.id, name: i.name, lat: i.lat, lon: i.lon }))
  const center = items[0] || { lat: 18.79, lon: 98.99 }
  const total = markers.length
  const fastCharge = items.filter((i) => (i.kw || 0) >= 50).length
  const acCount = Math.max(total - fastCharge, 0)
  const dcLabel = fastCharge > 0 ? `${fastCharge} แห่งรองรับ ≥50kW` : 'เน้น AC/ชาร์จช้า'
  return (
      ...
      <section className="relative overflow-hidden rounded-[32px] border border-slate-200/80 bg-white/90 p-6 shadow-xl shadow-slate-900/10">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pb-6">
          <div className="flex items-center gap-4">
            <BackButton fallbackHref={`/chargers`} />
        ...
        <div className="grid gap-4 lg:grid-cols-[1.3fr,0.7fr] lg:items-end">
          <p className="mx-auto max-w-3xl text-center text-lg leading-relaxed text-slate-600">
            รวมสถานีชาร์จรถไฟฟ้าในเส้นทางจังหวัด {toThaiProvince(params.province)} ตรวจสอบจุดแวะชาร์จบนแผนที่ เลือกสถานีที่มีกำลังไฟเหมาะกับรถของคุณ แล้วกดดูรายละเอียดต่อได้ทันที
          </p>
          <div className="grid w-full grid-cols-2 gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-white/90 px-4 py-3 shadow-sm shadow-slate-900/5">
              ...
              <div className="text-xs text-slate-500">สถานีในแผนที่</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-sky-500 to-slate-900 px-4 py-3 text-white shadow-lg shadow-sky-500/30">
              <div className="text-xs font-semibold uppercase tracking-wide text-white/70">Fast / DC</div>
              <div className="text-2xl font-semibold">{fastCharge || 0}</div>
              <div className="text-xs text-white/80">{dcLabel}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white/90 px-4 py-3 shadow-sm shadow-slate-900/5">
              <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">AC / Destination</div>
              <div className="text-2xl font-semibold text-slate-900">{acCount}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.25fr,0.95fr] lg:items-start">
        <div className="flex h-[70vh] flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/60 lg:h-[calc(100vh-220px)]">
          <div className="flex items-center justify-between px-5 py-3">
            <div>
              <p className="text-sm font-semibold text-slate-500">แผนที่สถานีชาร์จ</p>
              <p className="text-xs text-slate-500">กดปักหมุดเพื่อดูชื่อสถานี</p>
            </div>
        ...
        <div className="space-y-4 lg:max-h-[calc(100vh-220px)] lg:overflow-y-auto lg:pr-1">
          <div className="rounded-3xl border border-slate-200/80 bg-white/95 p-4 shadow-sm shadow-slate-900/5">
            ...
            <QuerySearch placeholder="พิมพ์ชื่อสถานีหรือย่าน…" />
          </div>
          <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-4 shadow-sm shadow-slate-900/10">
            <div className="mb-3 flex items-center justify-between">
              <p className="text-sm font-semibold text-slate-700">รายการสถานี ({total})</p>
            ...
            <ListAccordion
              items={items.map((m) => ({
                ...
                title: m.name,
                subtitle: [
                  m.kw ? `${m.kw} kW` : 'กำลังไฟไม่ระบุ',
                  m.type || 'ประเภทไม่ระบุ',
                  (m.kw || 0) >= 50 ? 'DC / Fast' : 'AC / Destination',
                ].join(' • '),
                href: `/chargers/${params.province}/${m.id}`,
              }))}
            />
          </div>
        </div>
